...
class MyGame(arcade.Window):
    ...
    def on_joybutton_press(self, joystick, button):
        # Check for different integer for different attacks
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key == arcade.key.W:
            ...
            arcade.sound.play_sound(self.shoot_sound)
            self.bullet = Bullet(
                "images/midway/Shot 1.png", self.player_sprite)
            self.bullet_list.append(self.bullet)
            self.all_sprites_list.append(self.bullet)
    ...
    def update(self, delta_time):
        ...
        if self.player_sprite.health > 0:
            ...
            for enemy in hit_list:
                ...
                if self.player_sprite.health < 0:
                    Explosion(self.enemy_explosion_images,
    ...
    def on_draw(self):
        ...
        # 画得分情况
        score = "score: " + \
            str(self.score) + " , Blood volume: " + \
            str(self.player_sprite.health)
        arcade.draw_text(score, 10, 20, arcade.color.WHITE, 14)

        if self.player_sprite.health < 1:  # 血量小于1显示游戏结束
            self.interval_counter += 1
            if self.interval_counter < self.interval // 2:
                self.logo_1943.draw()
            if self.interval_counter % self.interval == 0:
                self.interval_counter = 0
...
class Cover(arcade.Window):
    ...
    def on_joybutton_press_for_cover(self, joystick, button):
        if button == 7:
            self.joystick.close()
            self.close()
